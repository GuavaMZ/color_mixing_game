...
class ChaosLabHUD extends StatelessWidget {
  ...
  Widget _buildInstabilityMeter(BuildContext context) {
    // Stability decays over time or with events
    final stability = game.chaosStability.clamp(0.0, 1.0);
  ...
  String _getCurrentDisasterText() {
    if (game.isBlackout) return 'POWER FAILURE DETECTED';
    if (game.isMirrored) return 'OPTICAL AXIS INVERTED';
    if (game.hasWind) return 'HIGH PRESSURE LEAK';
    if (game.isLeaking) return 'STRUCTURAL BREACH';
    if (game.isControlsInverted) return 'INTERFACE REVERSED';
    if (game.isEvaporating) return 'THERMAL OVERLOAD';
    if (game.isGravityFlux) return 'GRAVITY ANOMALY';
    if (game.isUiGlitching) return 'DATA CORRUPTION';
